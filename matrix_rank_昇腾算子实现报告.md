...
>
> **阅读指引**:第一至三章是设计/优化的主体内容,其中 §1.4 已更新为**当前最终 dispatch**;第一阶段的原始结构保留在附录 C。第二至第十一阶段是按时间顺序的演进记录(RRQR → 精确大矩阵路径 → hermitian 基线修正与小矩阵优化 → 评审修复与工具链退化处置 → 精确路径推广 → 默认分发切换 → 三轮评审修复 → 精确路径优化评估 → 五方向优化意见闭环与 slack 裁剪落地),各阶段内的"当前状态"描述以写作时为准,最终以 §0 总结 + §1.4 + 第十、十一阶段为准。
...
| 65~255(herm 与非 herm)| **无主元 Householder QR**(例外②,历史章节中称 RRQR——实际无主元,名不副实,此处更正)| 精确路径在该区间有 tile 边界低谷(见下),不达 0.8 底线;语义缺口:无主元 QR 非秩揭示分解,|R_ii| 系统偏离 σ_i——缓衰减谱低估,且实测普通 randn + 中谱 atol 也低估(256² 报 94/真值 129,~-27%);近阈值容差输入应走 `FLAGGEMS_MR_EXACT_PATH=1` |
...
linalg_matrix_rank(input, *, atol, rtol, hermitian)
  ...
  └─ _launch_matrix_rank: k = min(m,n), rows = max(m,n), batch_count = numel/(m·n)
       ...
       │         (精确但慢,见第六阶段 §3 的性能墙分析)
       ├─ hermitian 且 k ≥ 256 → _launch_tridiag_big_rank(默认,精确):
       ...
       ├─ 非 herm 且 k ≥ 256 → _launch_bidiag_rank(默认,精确):
       │    非分块 Golub-Kahan 双对角化(尾随裁剪 grid + 单遍 step)+
       │    独立 BᵀB 构造 kernel + df64 Sturm;NPUGraph 捕获
       │    (256² 1.10 / 512² 1.75 / 1024² 2.6×)
       │    ※ 两条路径在 65~255 经 FLAGGEMS_MR_EXACT_PATH=1 也可强制(验证用)
       └─ 65 ≤ k ≤ 255(herm 与非 herm)→ 分块 Householder QR(无主元,历史
            章节称 RRQR):rank 由 |R_ii| 对角读出。精确路径在 129~192 有
            tile 边界性能低谷(general 129² 0.50、herm 129² 0.72),0.8 底线
            达不到,故阈值定在 255;QR 的缓衰减谱低估(6× 余量内也会发生)
            维持文档化——这是默认分发中仅剩的 QR 语义缺口
```
