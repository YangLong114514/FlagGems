...
> 提交:`ea2f674b`(第十一阶段:slack 裁剪落地 + 五方向评估闭环)
...
**默认分发(第十二阶段起,全部精确线性域;两段快速近似改为 env 选择加入)**:
...
**为什么这是正确选择**:
1. **线性精度**:双对角化的 d/e 后向误差 ~ `K·eps·‖A‖`(与 σmax 同量级);BᵀB 的 dd/ee 是 d/e 的标量运算(相对误差 ~eps)。**这消除了 Gram 的 √eps·σmax 地板**——σmin 的可分辨下限从 √eps·σmax 降到 eps·σmax 量级(分解阶段后向误差的固有地板,df64 Sturm 无法恢复 fp32 分解已损失的信息),偶发错误随之消失。实测 260/260 全对。
...
linalg_matrix_rank(input, *, atol, rtol, hermitian)
  ...
  └─ _launch_matrix_rank: k = min(m,n), rows = max(m,n), batch_count = numel/(m·n)
       ...
       │    (herm 256² 2.04 / 512² 3.97 / 1024² 7.11)
       ├─ 非 herm 且 k > 64 → _launch_bidiag_rank(默认,精确):
...
**至此 exact 路径(env 开)在 benchmark 65~512 频段的全部采样 shape(128/256/512 方阵及 512×1024/1024×512,general + herm)上 ≥0.8**(general 128² = 0.82 压线,其余 1.1~1.75;herm 1.34~3.97);65~127 小区间经探针测量仍低于 0.8(0.47~0.85,见第七阶段切换边界),唯一剩余的性能缺口是长维 k≤64 的 QR 压缩路径(0.29~0.61,panel 串行 GM 往返的墙,见第六阶段 §3)。

## 7. 验证(第六阶段最终态)

- 官方套件:**92 passed / 6 skipped**(新增:8 个精确路径测试 + 11 个非方阵回归测试)
- 366 例扫描(默认 dispatch):48 失败 = 46 长维 Gram lowrank(已知地板,env 精确路径下全消)+ (3,3)/(7,7) fp32 噪声区 2 例(后经第八阶段完整清单核对)
...
**⚠ 第八阶段修正(扩展 benchmark 揭示)**:按评审建议把 65~192、129 边界、129×512 等 shape 加入 benchmark 后,发现精确路径在 **129~192 存在 tile 边界性能低谷**(general 129² 0.50、160² 0.69、192² 0.90,非方阵 129×2048 0.52;herm 65² 0.74、129² 0.72)——k=129 跨过第二个 64 宽 tile 边界,每步成本跳升,而此前只采样了 128(恰在边界内)和 256。切换阈值因此从 128 上修到 **255**(k ≥ 256 才默认精确路径:general 256² 1.10、herm 256² 2.04 有余量;65~255 两个语义域都回 QR)。上文的三条变更为修正前的历史记录,以第八阶段为准。

验证(切换后):366 例默认扫描 48 失败(46 例外① Gram 地板 + (3,3)/(7,7) fp32 噪声区 2 例——完整清单见第八阶段核对,与切换前一致);exact 扫描 2 失败(fp32 噪声区);herm 压力 34/34(**默认模式**);官方套件双模式 92 passed / 6 skipped;benchmark 默认分发 48 项全部 SUCCESS(general ≥0.93,herm 除两个历史边际 shape 外 ≥0.89)。

切换边界测量(general exact,墙钟中位数):(65,65) 0.475、(80,80) 0.608、(96,96) 0.676、(100,100) 0.699、(112,112) 0.742、(128,128) 0.847。

---

# 第八阶段:评审修复第二轮(严格阈值/测试进仓库/NPUGraph 加固)

> 背景:第二轮外部评审指出 ① hermitian 严格阈值语义错误(correctness blocker)② 扫描脚本未进仓库无法复现 ③ NPUGraph 缓存的并发/冷启动生产风险 ④ benchmark 缺切换点采样 ⑤ 报告若干不一致。

## 1. hermitian 严格阈值(#{|λ| > tol} 的负侧必须严格)

torch 语义:rank = #{λ > tol} + #{λ < −tol}(两侧都严格)。Sturm qd 递推的零主元约定(q==0 换成 tiny 负值)实际统计 #{λ ≤ x}:正侧 K − #{λ ≤ tol} 恰好是严格的 #{λ > tol},但负侧 count_le(−tol) 把 λ == −tol 也计入。两个确定性反例(修复前全部复现):diag([1, −0.5, 0…]) atol=0.5 报 2 应为 1;diag([1, 0, …]) atol=rtol=0 报满秩应为 1(此前的 `if tol==0` 捷径只处理了全零谱,处理不了非零缺秩谱)。

修复:负侧统一经 `_neg_strict_shift(tol)` 求"恰好小于 −tol 的阈值"——#{λ ≤ pred(−tol)} 在 fp32 格点上恰好等于严格的 #{λ < −tol}。四条路径全部修正(k≤32 融合、33~64 padded、大 herm 三对角的 bracket 与 df64 final;非 herm 双对角路径的 σ² 域计数本来就是严格大于,不受影响)。df64 final 的 tol==0 捷径随之删除(pred(−0.0) = −1.4e-45 天然给出 #{λ < 0})。

**新工具链缺陷(补充 §4.1)**:标量 bitcast(`x.to(tl.int32, bitcast=True)`)在此后端误编译——nextafter 的位操作实现下 −0.0 会产出 NaN 阈值(实测:非零缺秩谱负侧计数归零,报 1 应为 2)。改用算术形式:tol>0 时 −tol×(1+2eps)(精确乘积落在后继区间,舍入后严格小于 −tol),tol==0 时硬编码 −1.4e-45。

## 2. 连带发现:三对角 apply 的 tau² 溢出(强 deflate 谱 NaN)

调试上述反例时发现更深的预存 bug:密集矩阵谱强烈收缩时(本例 ±0.5 打结 + 124 个近零特征值),尾部次对角 ~1e-10 → 反射 tau = 2/vnorm2 ~ 1.4e20,`0.5*tau*tau*cs` 中 tau² = 2e40 溢出 fp32(>3.4e38)→ W 对角被写成 −inf → 下一步浮点掩码 0×inf = NaN 污染整条三对角 → 近满秩/NaN 结果。修复:apply kernel 改算 `0.5*tau*(tau*cs)`(数学等价,永不对 tau 平方)。该 bug 影响所有 herm k>64 输入(此前的 herm 压力测试恰好没有触发足够小的尾部次对角)。已补 deflate 谱回归测试(k=65/128/257/513)。

## 2.5 切换阈值的二次修正(128 → 255)与两个潜伏编译 bug

按评审建议把 65~192、129 边界、129×512 等 shape 加入扩展 benchmark 后,发现三处此前从未触达的问题(这些 shape 从未在 RRQR 上跑过——旧 benchmark 从 128 直接跳 256):

1. **精确路径在 129~192 有 tile 边界低谷**(general 129² 0.50、160² 0.69、192² 0.90、129×2048 0.52;herm 65² 0.74、129² 0.72):k=129 跨过第二个 64 宽 tile 边界,每步成本跳升;此前只在 128(边界内)和 256 采样,漏掉了低谷。**默认切换阈值从 128 上修到 255**(herm 与 general 一致):65~255 回无主元 QR,k ≥ 256 默认精确(general 256² 1.10、herm 256² 2.04,有余量)。
2. **reg panel kernel 的 NB=3 特化 UB 分配抖动**:`nb = max(1, rs//64)` 对 rows∈(128,256] 产生 NB=3,与 kernel 注释声明的定义域 {1,2,4} 不符;NB=3 的 UB 分配在 244KB/192KB 边界上摇彩票((129,129) 失败、(192,192) 通过,同一特化)。修复:`nb = min(4, next_power_of_2(rs//64))`,NB=3 永远不再出现。
3. **B=1 整型特化 UB 溢出**:JIT 把等于 1 的整型参数当 constexpr 特化;k≡1 (mod 64)(65/129/193)的尾 panel B=1 触发一个从未编译过的特化,NB=4 下确定性 UB overflow(k=129 失败而 k=131 通过)。修复:b==1 的 panel 改走 GM-tile panel kernel(每个这样的 k 只多一次 GM panel 调用)。

## 3. 测试进仓库(评审问题 2)

- `tests/test_linalg_matrix_rank.py` 新增 5 组:herm 严格阈值(k=3/33/65/128/257 × 正负打结/atol=rtol=0/batch 逐容差,65/128 例固定 EXACT env 以覆盖精确 herm 路径)、deflate 谱溢出回归、dispatch 边界(general 128(QR,清晰谱)/255(QR)/256(精确,缓衰减)/256×512/512×256/batch,herm 64(padded)/65(QR)/256(三对角))、graph vs FLAGGEMS_MR_NO_GRAPH=1 逐位对拍(256/513)、大 herm 垃圾上三角(300/513)。
- `tests/test_linalg_matrix_rank_sweeps.py` 新增:评审引用的三套扫描的仓库可复现版(366 例全路径、herm 34 例、QR 频段对抗 22 例),`FLAGGEMS_MR_SWEEP=1` 时启用(默认跳过,避免拖慢套件);366 例的允许失败集(长维 Gram 地板 +(3,3)/(7,7) 噪声区)在断言中显式枚举。
- 边界测试的一个实测修正:无主元 QR 在 6.5× 容差余量下仍低估缓衰减谱(σ=1e-4 vs tol=1.5e-5 报 59/60)——比"近阈值 ±1"更弱,文档措辞已同步;RRQR 边界用例改用清晰间隙谱。

## 4. NPUGraph 加固(评审问题 3)

- 缓存 key 加入当前 stream 句柄(capture 只在捕获 stream 上有效);两个缓存的查找/捕获/staging/replay 全程用模块级 `threading.Lock` 串行化(此前同 shape 并发调用会互相覆盖 staging/tol/out)。
- 逐出策略从"满 16 整体清空"改 FIFO 单条逐出(shape churn 只重捕一条)。
- herm 改用精简 `_tridiag_workspace`(去掉 bidiag 专用的 U/ddbuf/eebuf/taur,每缓存条目省 ~40% 显存)。

## 5. 遗留(评审接受范围内的已知边界)

- graph 冷启动(首 shape 的 warmup+sync+capture)成本仍存在;benchmark 数字为稳态。实测冷/暖对比:(256,256) 冷 1925ms/暖 14.1ms(general)、冷 776ms/暖 7.0ms(herm);(512,512) 冷 1979ms/暖 46.8ms(general)、冷 1101ms/暖 20.7ms(herm)——冷启动含 JIT 编译(可缓存跨进程复用)与一次性 capture。
- 多 stream 并发已按 key 隔离,但同 stream 多线程调用经锁串行(不并行加速,正确性优先)。
...
# 第十阶段:精确路径(65~255 频段)优化评估——两条严格排除、两条估算不足,四个候选方向待真机

> 背景:精确路径(GK 双对角化)在 65~255 频段为 0.42~0.90×(129² 低谷 0.53),评估能否优化到 0.8 以上。**精确结论**:在现有 UB 容量、编译器约束和 GK 实现框架下,尚未找到能让所有 65≤k≤255 shape 稳定 ≥0.8 的低风险方案;四个已评估方向中两条被严格排除(本节 §1 数学证伪、§2 真机 UB 实测),另外两条(§3/§4)是预期收益不足的估算结论,**并未在算法上证明所有精确方案均不可行**。

## 1. 严格排除:QR + 仅右反射 R 双对角化(数学不成立)

设想:先走已有的快速 QR 得 k×k 上三角 R(σ(R)=σ(A),线性域),再对 R 只做右反射链双对角化(三角阵左反射"看似"恒等)。**纸面证伪**:右反射混合列 j+1..k-1 的全部行,会向下三角引入填充——3×3 手算即见:对 R=[a b c;0 d e;0 0 f] 第一步右反射后 (2,1) 位置变为非零。该填充其后没有任何反射会清除,单边反射不可能完成双对角化,左反射省不掉。LAPACK 对三角阵的双对角化同样是双侧的。

## 2. 严格排除:BLOCK=128 寄存器驻留全 GK(UB 硬墙,两个变体实测)

k≤128 时整个矩阵 65KB 理论可驻留 UB,无需 panel/GEMM。两个变体真机探针(均死在 `ConvertLinalgRToBinary`,即 UB 分配失败):

- **g + gT 双瓦片**(bidiag64 的直接放大,绕开 axis=0 归约缺陷):~260KB,超 192KB;
- **去 gT、axis=0 行提取**(单 g 65KB + 提取/外积临时瓦片):~195KB,仍超。

证实第六阶段死路①(128 宽寄存器 GK)的根源不是双缓冲策略,而是 **(128,128) fp32 瓦片操作数本身超 UB**——任何全矩阵驻留方案在 k>64 都不成立。

## 3. 估算不足(未真机证伪):panel 块化 GK(DLABRD 式双侧块化)

垂直 panel(rows×64)+ 水平 panel(64×k)双驻留,panel 内 64 步延迟更新,尾随矩阵两侧 GEMM(WY 需要双份反序 T 矩阵)。按 QR reg panel 的实测单位成本外推:GK panel 每步工作量约为 QR panel 的 2 倍(左右双侧 + 行刷新),129² 估算 ~5~6ms,**0.65~0.8×,大概率越不过底线**;且 (129,2048) 类长行 shape 的水平 panel 超 UB,依然无解。工程量是重写一条路径(数天),编译彩票风险高。**这是成本外推结论,不是真机证伪**;以当前估算不投。

## 4. 对验收无效(非不可行):提前 deflate

秩计数只需要"超过 tol 的个数",理论上尾随块 F 范数 ≤ tol 即可提前退出主循环。但 benchmark 用满秩 randn,tol = k·eps·σmax 极小,不会发生 deflate——对验收数字零增益;对低秩/快衰减谱仍可能有效,但不能解决验收性能问题。

## 5. 候选方向的真机/离线验证结果(全部已闭环)

1. **带证书的快速 QR + 精确回退——离线研究证伪(不再首选)**:界本身严格(σ_r(A) ≥ σ_min(R11) 子矩阵交错;σ_{r+1}(A) ≤ ‖R22‖_F),但**无主元 QR 不是秩揭示分解**:满秩 randn 256² + 中谱 atol 下,‖R22‖_F 是 atol 的 12.7 倍(下证书结构性失败);δ_QR = 10·k·eps·‖A‖_F 与默认 tol = k·eps·σmax 同阶,缺秩矩阵的下证书永不成立(δ > tol_lo 恒成立)。离线命中率(6 个 k × 4 谱族 × 2 容差 × 5 seed):**仅满秩 randn 默认容差 60%,其余全部 0%**——证书只在 QR 本来就正确的输入上命中,在语义缺口咬人的地方命中率恒为 0。带列主元 QR 能救证书,但主元选择占 panel 时间 ~55%(第二阶段实测),全频段会跌破 0.8。**附带发现(比文档原措辞更严重)**:无主元 QR 的 |R_ii| 计数对普通 randn + 中谱 atol 低估约 27%(256² 报 94,真值 129)——QR 段的语义缺口不只是"缓衰减谱",任何近阈值容差都会触发,§0 例外②的措辞已按此更新。
2. **TSQR 治长维 k≤64——真机探针,不过线,停**:复用 reg panel + extract kernel,叶子并行 + 两两归并,NPUGraph 重放计时(排除 host dispatch)。正确性优秀(σ 相对误差 ~3e-7,线性域),但关键路径 = 归并层数 × 64 步串行反射链(每步 ~15~25μs tile-op 延迟,叶子并行消不掉层内串行):(512,64)/(1024,64) lb=256 时 3.01/4.19ms(0.77×),加上 bidiag64+Sturm 尾巴后 ~0.6~0.7×;lb=128(三层)更差。低于停手线 1.25×T_torch,**停**。
3. **两阶段双对角化(dense→band→bidiagonal,研究级)**:第一阶段 Cube GEMM 化带可行;但 bulge chasing 是细粒度强依赖流水线,在"无网格屏障 + tile 发射是地板"的本后端是最差匹配模式,k≤255 规模下第二阶段开销很可能吃掉第一阶段收益。只允许先做一个 panel 微基准。
4. **分块 one-sided Jacobi(研究备选)**:列小块并行旋转 + NPUGraph 轮间同步,线性域。除聚簇谱收敛风险外,还有一个结构性障碍:Jacobi sweep 轮数数据依赖,而 NPUGraph 捕获要求静态 launch 序列——要么固定轮数过迭代,要么每轮 host 同步;另见第五阶段 GM 驻留 sweep 的 MTE 重排尸检。

## 结论与最终立场

当前已评估的方向均无法在现有硬件、工具链和工期约束下让 65~255 频段(及长维 k≤64)的精确路径稳定达到 0.8:两条严格排除(§1 数学、§2 UB 实测),两条实证/估算不足(§5.1 离线命中率、§5.2 真机、§3 外推),其余为研究级。默认分发维持现状:65~255 走无主元 QR(0.93~1.9×,语义缺口已按 §5.1 的附带发现更新措辞),`FLAGGEMS_MR_EXACT_PATH=1` 是语义优先用户的 opt-in。

# 第十一阶段:五方向优化意见的评估闭环(方向 2 落地,exact 频段 +8~10%;方向 1/3/5 真机证伪)

> 背景:外部评审提出五个仍可评估的精确路径优化方向:①长矩阵 QR→R→完整 GK(R);②去除 slack tile 计算;③65/129/193 边界混合尾块 kernel;④scratch partial 替代 atomic 累加;⑤NB=8/16/32 小 panel blocked GK。本阶段逐条真机闭环。

## 1. 方向 1(QR → R → 完整 GK(R))——真机证伪(数学成立,性能不成立)

σ(A)=σ(R),语义上完全成立,证伪的是性能。真机时间拆分(2048×129,baseline 13.0~14.2ms):

| 组成 | 实测 |
|---|---|
| 现有 QR(rows=2048,GM panel)| 32.4ms |
| 精确 GK(129² 方阵)| 6.8ms |
| **QR + GK(R) 合计(估算下界)**| **39.2ms → 0.36×** |
| 对比:default(RRQR)32.4ms → 0.44×;exact 全矩阵 GK 22.7ms → 0.58~0.62× | |

QR+GK(R) 比 default 和 exact **都慢**。原因:46% 的 FLOP 论证在本硬件不迁移——瓶颈是 GM panel 单 program 串行反射链(rows=2048 时 64 步 × 32 行块串行)与每 tile 发射地板,不是 FLOP;而"快速长维 QR"这个前提不存在(TSQR 探针已停于 0.77×,GM panel 更慢)。(512,129)/(256,129) 同结论(0.46~0.47×)。

## 2. 方向 2(slack tile 裁剪)——核实属实,已落地

大矩阵 workspace 为无掩码 tile 寻址多分配一个 64 宽 slack(`rs/kp = cdiv(·,64)*64 + 64`),核实发现三处把它算进了计算:

- `_bidiag_run` 右侧 `ntr = cdiv(rs-1-j, 64)`:把 slack 与 pad 行全算进 rmat/rapply 网格。近方阵 129² 在 j=0 时右侧 tile 4 个 vs 有效 2 个(**右侧工作量翻倍**);
- `_mr_bidiag_lstep_kernel` 串行循环跑到 `RS//64`,多处理一个全零 slack 块;
- `_mr_rrqr_init_kernel` 初始化循环写 slack 块。

修复:ntr 改按 `rows` 计算;lstep 新增 RBV=cdiv(rows,64) 参数;init 循环界改 `(ROWS+63)//64`(对 RRQR/longdim 路径 rs 无 slack,行为不变)。安全性:pad/slack 行恒零(zeros workspace + init 掩码写;lapply/rapply 的更新项 v/u 在 pad 行恒为 0),NPUGraph 重放下该性质不变;裁剪后无任何 kernel 读 slack tile。

实测(exact 路径,前后对比):129² 6.83→6.18ms(0.55→0.61×)、160² 0.70→0.78×、192² 0.92→1.01×、256² 1.04→1.16×,**稳定 +8~10%**;长维 (2048,129) 不变(slack 占比 1/33,符合预期)。默认分发下 k≥256 的 exact 频段直接受益。不改变数值语义。

## 3. 方向 3(混合尾块 kernel)——微基准证伪

问题本质:tile 发射地板按 tile **个数**还是**宽度**计费?微基准(rank-1 更新 kernel,NPUGraph 捕获排除 host 开销):

| tiles | 64 宽 | 32 宽 | 16 宽 |
|---|---|---|---|
| 1 | 1.32µs | 1.28µs | 1.39µs |
| 64 | 3.64µs | 3.72µs | 4.01µs |

**同数 tile 下宽度无关**——成本是每次发射的固定开销。把 65/129/193 的几乎全空尾块换成 16/32 宽零收益;唯一有效手段是减少 tile 数,方向 2 已做。停。

## 4. 方向 4(scratch partial 替代 atomic)——分析排除,不投

每列 atomic 贡献者 ≤ NRC(65~255 频段 2~4 个),争用可忽略;§3 微基准证明地板在每次发射的固定成本而非累加方式;scratch 方案不减 launch 数(apply 改为读 2~4 份 partial),反而多一层 GM 往返。已知 atomic 误编译风险已由 TRITON_ALL_BLOCKS_PARALLEL pop+_GRAPH_LOCK 闭环。重写四条最敏感 kernel 换不确定收益,风险不对等。

## 5. 方向 5(NB=8/16/32 小 panel blocked GK)——真机探针,按停手规则停止

以现有 GM panel kernel 作 panel 分解替身,真机测 B=16、rows=2048 的单 panel:**4.0ms**(16 步串行反射,~250µs/步)。129 列分 8 块的完整估算 ~36.5ms → 0.39×(baseline 14.2ms)。即使乐观假设专用 lean panel(只更新 16 列,比替身精简 ~2.5×),估算也仅 ~0.9~1.1×——贴线,且要重写整条路径(WY 双 T + Cube GEMM 尾随更新 + 全新正确性面)。按既定停手规则(panel+一次尾随更新无 >0.9× 余量即停),**停**。该探针同时反证了方向 1 的前提:2048 行 panel 的串行反射链就是长维 QR 慢的根因。

## 6. 结论

五方向闭环:方向 2 落地(exact 频段 +8~10%,默认分发 k≥256 直接受益,无语义变化);方向 1/3/5 真机证伪,方向 4 分析排除。长维中 k((129,2048) 类)的 0.38~0.44× 维持已知结构缺口:长维 QR 的串行反射链与 exact GK 的每步发射地板在现有工具链下均无低风险解。65~255 频段的默认 QR / opt-in exact 分发维持不变。

**验证**:官方套件 130 passed / 44 skipped(sweep 文件 38 例默认 skip + fp64/complex 环境跳过)0 失败;benchmark 67/67 正常执行,general 算术 2.34/几何 1.90,herm 2.11/1.69(与第八~十阶段基线在共享机器抖动内一致;小 shape (256,8)/(512,16) 本轮 0.55/0.83 经单独复测为 0.98/1.25,确认是机器抖动);sweeps 3/3(366 全路径 + herm 34 exact + QR 频段对抗 22 exact)全过——366 默认分发、34/22 内部固定 `FLAGGEMS_MR_EXACT_PATH=1`,直接覆盖被裁剪的 bidiag/tridiag 大路径(含非方阵、batch、近阈值谱)。
...
- 长维 k≤64(`_launch_tridiag_rank` 内):`FLAGGEMS_MR_FAST_PATH != "1"` → 精确 QR 压缩 + bidiag64 + df64 尾巴(**默认**);`== "1"` → Gram(Cube)+ 三对角化 + Sturm 快路径。
- k > 64(`_launch_matrix_rank` 内):`k > 255 or FLAGGEMS_MR_FAST_PATH != "1"` → 精确路径(herm 单边三对角化 / 非 herm GK 双对角化,**默认**);否则(65~255 且 FAST_PATH=1)→ 无主元 Householder QR。
- `FLAGGEMS_MR_EXACT_PATH` 不再被代码读取(精确已是默认,设置它无害);`FLAGGEMS_MR_FAST_PATH` 与 EXACT 一样在 dispatch 时逐次读取,运行中可切换。
- 模块 docstring、`_RRQR_MAX_K` 注释、Gram kernel docstring、`_launch_bidiag_rank` docstring 同步反转表述。
- 测试逻辑零改动:此前固定 `FLAGGEMS_MR_EXACT_PATH=1` 的用例在新默认下语义不变(env 冗余但无害),仅更新注释说明。366 扫描的长维 lowrank 允许清单保留(快速模式下仍有 Gram 地板),新默认下这些用例直接通过。

## 2. 性能(本轮实测,`benchmark --mode operator`,67/67 正常执行)

| 口径 | general | herm |
|---|---|---|
| 精确默认(新) | 算术 1.82 / 几何 1.28 | 算术 2.15 / 几何 1.74 |
| 快速路径(旧默认,FAST_PATH=1 等价) | 算术 2.40 / 几何 1.98 | 算术 2.12 / 1.71 |

精确默认下 <0.8 共 20 项,全部是此前各阶段已记录的结构性缺口,与本次反转无关:

- **长维 k≤64 八项(0.30~0.59)**:(32,1024) 0.30、(16,512) 0.40、(64,512) 0.49 等——放弃 Gram 快路径的代价,瓶颈是 QR panel 的 O(k) 步串行 GM 反射链(第六阶段 §3、第十一阶段方向 1/5 已证伪全部已知替代)。
- **近方阵 65~160**:(65,65) 0.45、(80,80) 0.55、(96,96) 0.61、(112,112) 0.78、(129,129) 0.58、(160,160) 0.79——tile 边界低谷(第八阶段 §2.5、第十一阶段方向 3 已证伪尾块方案);192² 起恢复(0.96),256² 1.14。
- **长维 k≥129**:(129,512) 0.76、(129,2048) 0.53、(2048,129) 0.57——长行中 k 固有地板(两条路径同源)。
- **herm 边际**:33² 0.74、65² 0.71(小 shape 发射延迟区间,多轮 0.71~0.98 抖动)。

整体算术平均 1.92、几何 1.40,仍显著高于 0.8;大矩阵精确路径:general 1024² 2.80×、512² 1.78×;herm 1024² 6.95×、512² 3.80×。需要性能优先的调用方设 `FLAGGEMS_MR_FAST_PATH=1` 即恢复旧默认分发(上表第二行)。
