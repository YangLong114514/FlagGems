...
| 65~255(herm 与非 herm)| **无主元 Householder QR**(例外②,历史章节中称 RRQR——实际无主元,名不副实,此处更正)| 精确路径在该区间有 tile 边界低谷(见下),不达 0.8 底线;缓衰减谱低估(6× 余量内也会发生)文档化,env 可选精确路径 |
...
## 5. 尚未被真机排除的候选方向(按投入产出排序)

1. **带证书的快速 QR + 精确回退(首选,语义升级而非加速)**:快速 QR 给出候选秩 r;将 R 分块为 [R11 R12; 0 R22],利用子矩阵奇异值交错(σ_r(A) ≥ σ_min(R11))与 Courant–Fischer(σ_{r+1}(A) ≤ ‖R22‖₂ ≤ ‖R22‖_F)构造证书:σ_min(R11) 下界 > tol 上界 且 ‖R22‖_F + δ_QR < tol 下界 ⇒ 安全返回 r;否则回退精确 GK。清晰谱保持 QR 性能,可疑谱自动精确化,默认语义不再依赖 |R_ii|≈σ_i。**实现要点**:σ_min(R11) 的严格下界需要三角逆(1/‖R11⁻¹‖_F,O(r³/3),新 kernel,中等彩票风险);回退分支需要一次 host 同步(或 device 侧早退,仍付 graph 空转)。先做离线命中率统计再写 kernel。
2. **TSQR 治长维 k≤64(零件全部现成)**:叶子块(128/256×64)正好落在 `_mr_rrqr_panel_reg_kernel` 定义域,`_mr_extract_upper_kernel` 提取各叶 R,两个 64×64 R 堆叠成 128×64 再归并仍用同一 panel kernel;叶子并行直接消掉串行 GM 往返的墙。最小原型只测 (512,64)/(1024,64)/(64,512) 的 R 正确性与耗时,目标 T_gems ≤ 1.25·T_torch,不达即停。
...
当前已评估的四种优化方案均无法在现有硬件、工具链和工期约束下稳定满足性能要求,暂不继续投入。**该结论不代表精确算法在理论上不存在其他实现方式**;带证书的快速路径、TSQR、两阶段双对角化和分块 Jacobi 仍是尚未被真机排除的候选方向,按 §5 的顺序评估。默认分发维持现状:65~255 走无主元 QR(0.93~1.9×,语义缺口已文档化),`FLAGGEMS_MR_EXACT_PATH=1` 是语义优先用户的 opt-in。
