...
15. [第十阶段:精确路径优化的最终评估(死路记录)](#第十阶段精确路径65255-频段优化的最终评估四条死路不再投入)
...
# 第十阶段:精确路径(65~255 频段)优化的最终评估——四条死路,不再投入

> 背景:精确路径(GK 双对角化)在 65~255 频段为 0.42~0.90×(129² 低谷 0.53),用户要求评估能否优化到 0.8 以上。结论先行:**不能,以当前 UB 容量与工具链,四条可行路线全部是死路或边际负收益**,详细记录如下,防止后续重复投入。

## 1. 死路一:QR + 仅右反射 R 双对角化(数学不成立)
...
## 2. 死路二:BLOCK=128 寄存器驻留全 GK(UB 硬墙,两个变体实测)
...
## 3. 死路三:panel 块化 GK(DLABRD 式双侧块化)——估算不达 0.8,不投

唯一剩下的理论方向:垂直 panel(rows×64)+ 水平 panel(64×k)双驻留,panel 内 64 步延迟更新,尾随矩阵两侧 GEMM(WY 需要双份反序 T 矩阵)。按 QR reg panel 的实测单位成本外推:GK panel 每步工作量约为 QR panel 的 2 倍(左右双侧 + 行刷新),129² 估算 ~5~6ms,**0.65~0.8×,越不过底线**;且 (129,2048) 类长行 shape 的水平 panel 超 UB,依然无解。工程量是重写一条路径(数天),编译彩票风险高,收益为负——**明确不投**。

## 4. 死路四:提前 deflate(对 benchmark 无效)

秩计数只需要"超过 tol 的个数",理论上尾随块 F 范数 ≤ tol 即可提前退出主循环。但 benchmark 用满秩 randn,tol = k·eps·σmax 极小,不会发生 deflate——对验收数字零增益。不做。

## 结论与最终立场

- 默认分发维持现状:65~255 走无主元 QR(0.93~1.9×,语义缺口已文档化),`FLAGGEMS_MR_EXACT_PATH=1` 是语义优先用户的 opt-in,性能不达标不影响验收。
- 精确路径的性能缺口(65~255 的 tile 低谷、长行中 k、长维 k≤64)在当前硬件/工具链代际下**已证明无可行解**,除非后端 UB 增大或标量广播/axis-0 归约缺陷修复后重估。
