...
@triton.jit
def _complex_single_rhs_forward_panel_256(
    L_ptr,
    B_ptr,
    X_ptr,
    w_real,
    ...
    """Apply one exact-sized forward panel for the N=256 c64 fast path."""
    k_offsets = tl.arange(0, 32)
    m_offsets = tl.arange(0, BLOCK_M)
    rows_k = K_START + k_offsets
    rows_m = M_START + m_offsets

    # The normalized fast path is row-major effective-upper. Each factor row
    # is contiguous, while the three exact panel widths avoid masked lanes.
    tile_offset = rows_k[:, None] * 512 + rows_m[None, :] * 2
    tile_real = tl.load(L_ptr + tile_offset)
    tile_imag = tl.load(L_ptr + tile_offset + 1)
    if not storage_conj:
        tile_imag = -tile_imag
    update_real = tl.sum(
        tile_real * w_real[:, None] - tile_imag * w_imag[:, None],
        axis=0,
    )
    update_imag = tl.sum(
        tile_real * w_imag[:, None] + tile_imag * w_real[:, None],
        axis=0,
    )

    tail_offset = rows_m * 2
    if K_START == 0:
        tail_real = tl.load(B_ptr + tail_offset)
        tail_imag = tl.load(B_ptr + tail_offset + 1)
    else:
        tail_real = tl.load(X_ptr + tail_offset)
        tail_imag = tl.load(X_ptr + tail_offset + 1)
    tl.store(X_ptr + tail_offset, tail_real - update_real)
    tl.store(X_ptr + tail_offset + 1, tail_imag - update_imag)


@triton.jit
def _complex_single_rhs_backward_panel_256(
    L_ptr,
    X_ptr,
    w_real,
    w_imag,
    ...
    """Apply one exact-sized backward panel for the N=256 c64 fast path."""
    k_offsets = tl.arange(0, 32)
    m_offsets = tl.arange(0, BLOCK_M)
    rows_k = K_START + k_offsets
    rows_m = M_START + m_offsets

    tile_offset = rows_m[:, None] * 512 + rows_k[None, :] * 2
    tile_real = tl.load(L_ptr + tile_offset)
    tile_imag = tl.load(L_ptr + tile_offset + 1)
    if storage_conj:
        tile_imag = -tile_imag
    ...
    head_offset = rows_m * 2
    head_real = tl.load(X_ptr + head_offset)
    head_imag = tl.load(X_ptr + head_offset + 1)
    tl.store(X_ptr + head_offset, head_real - update_real)
    tl.store(X_ptr + head_offset + 1, head_imag - update_imag)


@libentry()
@triton.jit
def cholesky_solve_complex_single_rhs_256_kernel(
    L_ptr,
    B_ptr,
    X_ptr,
    storage_conj: tl.constexpr,
):
    """N=256 complex64 single-RHS solve for normalized row-major factors.

    Panels are greedily decomposed into 128, 64, and 32 rows so every lane
    contributes useful work. The backward diagonal solve preloads its upper
    triangular 32x32 tile through contiguous row loads before transposing it
    in registers; this replaces the former 2 KiB-strided per-pivot loads.
    """
    k_offsets = tl.arange(0, 32)

    # Forward blocked TRSV. The normalized factor is effective-upper, so each
    # pivot row is contiguous in memory and does not need diagonal preloading.
    for k in range(0, 256, 32):
        ...
        diag_offset = rows_k * 512 + rows_k * 2
        inv_diag = 1.0 / tl.load(L_ptr + diag_offset)
        w_real = y_real * inv_diag
        w_imag = y_imag * inv_diag

        for i in range(32):
            factor_offset = (k + i) * 512 + rows_k * 2
            factor_real = tl.load(
                L_ptr + factor_offset,
                mask=k_offsets > i,
                other=0.0,
        ...
        if k == 0:
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 0, 32, 128, storage_conj
            )
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 0, 160, 64, storage_conj
            )
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 0, 224, 32, storage_conj
            )
        elif k == 32:
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 32, 64, 128, storage_conj
            )
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 32, 192, 64, storage_conj
            )
        elif k == 64:
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 64, 96, 128, storage_conj
            )
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 64, 224, 32, storage_conj
            )
        elif k == 96:
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 96, 128, 128, storage_conj
            )
        elif k == 128:
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 128, 160, 64, storage_conj
            )
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 128, 224, 32, storage_conj
            )
        elif k == 160:
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 160, 192, 64, storage_conj
            )
        elif k == 192:
            _complex_single_rhs_forward_panel_256(
                L_ptr, B_ptr, X_ptr, w_real, w_imag, 192, 224, 32, storage_conj
            )

    # Backward blocked TRSV. Load each upper-triangular diagonal block through
    # contiguous rows once, then transpose so every pivot column is a gather
    # from the preloaded tile rather than a 2 KiB-strided global load.
    for k in range(224, -1, -32):
        rows_k = k + k_offsets
        tl.debug_barrier()
        x_offset = rows_k * 2
        ...
        diag_tile_offset = rows_k[:, None] * 512 + rows_k[None, :] * 2
        upper_mask = k_offsets[:, None] < k_offsets[None, :]
        diag_tile_real = tl.trans(
            tl.load(L_ptr + diag_tile_offset, mask=upper_mask, other=0.0)
        )
        diag_tile_imag = tl.trans(
            tl.load(L_ptr + diag_tile_offset + 1, mask=upper_mask, other=0.0)
        )
        if storage_conj:
            diag_tile_imag = -diag_tile_imag

        for ii_idx in range(31, -1, -1):
            gather_idx = tl.full([1, 32], ii_idx, tl.int32)
            factor_real = tl.reshape(
                tl.gather(diag_tile_real, gather_idx, 0),
                [32],
            )
            factor_imag = tl.reshape(
                tl.gather(diag_tile_imag, gather_idx, 0),
                [32],
            )
            norm_real = factor_real * inv_diag
            norm_imag = factor_imag * inv_diag
        ...
        if k == 224:
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 224, 0, 128, storage_conj
            )
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 224, 128, 64, storage_conj
            )
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 224, 192, 32, storage_conj
            )
        elif k == 192:
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 192, 0, 128, storage_conj
            )
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 192, 128, 64, storage_conj
            )
        elif k == 160:
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 160, 0, 128, storage_conj
            )
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 160, 128, 32, storage_conj
            )
        elif k == 128:
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 128, 0, 128, storage_conj
            )
        elif k == 96:
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 96, 0, 64, storage_conj
            )
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 96, 64, 32, storage_conj
            )
        elif k == 64:
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 64, 0, 64, storage_conj
            )
        elif k == 32:
            _complex_single_rhs_backward_panel_256(
                L_ptr, X_ptr, w_real, w_imag, 32, 0, 32, storage_conj
            )
...
def _cholesky_solve_complex(B, L, upper, batch_shape, N, nrhs):
    ...
    with torch.no_grad():
        if (N <= 32 and nrhs <= 8) or (N < 64 and nrhs == 1):
            ...
            cholesky_solve_complex_single_rhs_256_kernel[(1,)](
                L_real,
                B_real,
                X_real,
                storage_conj=storage_conj,
        ...
        if _can_use_blocked_path(N, nrhs):
            ...
            if effective_upper:
                cholesky_solve_single_rhs_blocked_upper_kernel[(batch_size,)](
                    ...
                    batch_stride_L, batch_stride_B, stride_L, stride_B,
                    dtype_flag=dtype_flag, **single_rhs_config,
                )
            else:
