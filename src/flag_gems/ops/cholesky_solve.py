...
def _get_portable_blocked_launch_config(dtype, N, min_dot_rhs=1):
    ...
    which already beat Torch by ~4x. fp64 uses explicit tl.sum products (no
    correct fp64 tl.dot lowering), where narrow BLOCK_RHS=2 tiles win: the
    solve is dominated by the serial block chain, and narrow tiles launch
    more CTAs to keep the device busy.
    """
    if dtype == torch.float64:
        return {
            "BLOCK_K": 16,
...
def _get_portable_complex_blocked_launch_config(dtype, N, min_dot_rhs=1):
    ...
    complex128 uses tl.sum products (no correct fp64 tl.dot lowering) with
    narrow BLOCK_RHS=2 tiles for CTA occupancy, mirroring the fp64 choice.
    """
