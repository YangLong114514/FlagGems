...
def _lu_factor_info_kernel(
    ...
    sentinel = K + 1
    # Detect both exact zeros and NaN (NaN can arise from 0/0 division in the
    # factorization kernel, e.g. when the input matrix is all zeros).
...
def linalg_lu_factor_ex_out(
    ...
    *,
    ...
    lu, piv, inf = linalg_lu_factor_ex(input, pivot=pivot, check_errors=check_errors)

    lu_out.resize_(lu.shape)
    pivots_out.resize_(piv.shape)
    info_out.resize_(inf.shape)
    lu_out.copy_(lu)
