...
def _make_input(shape, pivot, device):
    ...
    *batch, m, n = shape
    k = min(m, n)
    scaling = k ** -0.5
    L = (torch.randn(*batch, m, k, dtype=torch.float32, device=device) * scaling).tril()
    L.diagonal(dim1=-2, dim2=-1).fill_(1.0)
    U = torch.randn(*batch, k, n, dtype=torch.float32, device=device).triu()
...
def test_linalg_lu_factor(shape, pivot):
    inp = _make_input(shape, pivot, flag_gems.device)
