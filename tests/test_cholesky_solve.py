...
# Import from correct backend module
if IS_ASCEND:
...
CHOLESKY_SOLVE_RHS_BOUNDARY_SHAPES = [
    (64, 15), (64, 16), (64, 17), (64, 31), (64, 32), (64, 33),
]
CHOLESKY_SOLVE_UPPER_SHAPES = [
    (2, 1), (4, 2), (8, 4), (16, 8), (2, 16, 4), (8, 32, 8),
]
...
CHOLESKY_SOLVE_COMPLEX_SHAPES = [
    (2, 1), (4, 2), (16, 5), (33, 4), (64, 8), (2, 16, 4),
]
CHOLESKY_SOLVE_COMPLEX_BLOCKED_SHAPES = [
    (64, 1), (64, 4), (64, 33), (128, 1), (128, 16), (256, 1), (256, 128),
]
...
def _make_cholesky_solve_inputs(shape, dtype, matrix_scale=1.0, rhs_scale=1.0):
    *batch_dims, n, nrhs = shape
    ...
    rhs = rhs_scale * torch.randn(*batch_dims, n, nrhs, dtype=dtype, device=flag_gems.device)
    return A, L, rhs
...
def _make_conditioned_inputs(shape, dtype):
    *batch_dims, n, nrhs = shape
    ...
    eigs = torch.logspace(0.0, math.log10(condition), n, dtype=real_dtype, device=flag_gems.device)
    A = (Q * eigs) @ Q.mH
    L = torch.linalg.cholesky(A)
...
# ---------------------------------------------------------------------------
# Tests — complex types (GPU only, Ascend does not support complex)
# ---------------------------------------------------------------------------
...
def test_cholesky_solve_complex_broadcast(dtype, upper):
    A_shape = (2, 1, 4, 4)
    rhs_shape = (3, 4, 2)
    _, factor, rhs = _make_cholesky_solve_broadcast_inputs(A_shape, rhs_shape, dtype, upper)
    ref_out = torch.cholesky_solve(rhs, factor, upper=upper)
    res_out = _solve_with_gems(rhs, factor, upper=upper)
    utils.gems_assert_close(res_out, ref_out, dtype)
    assert res_out.shape == ref_out.shape
